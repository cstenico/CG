...
bool atingido = false, shoot = false, youwin = 0;
...
void bangbang()
{
    //printf("missel2_tx %f, missel2_y %f, aviao_x, %f \n", missel2_tx, missel2_y, aviao_x);
    if(missel2_moving && missel2_y <= -0.2 && missel2_y>=-0.3 && missel2_tx<aviao_x+0.5 && missel2_tx > aviao_x-0.5)
    {
        lifes--;
        if(lifes == 0) userLifes[7] = '0';
        if(lifes == 1) userLifes[7] = '1';
        if(lifes == 2) userLifes[7] = '2';
        if(lifes == 3) userLifes[7] = '3';
...
void move_missel2(int passo){
    ...
    if(missel2_y > -0.3 && missel2_moving)
    {
